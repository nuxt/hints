import { hintIssues } from '../../composables/devtoolsData'
...
enum ImagePerformanceIssueType {
...
export interface ImagePerformanceIssueDetails {
  type: ImagePerformanceIssueType
  // todo add component location
}

export default defineNuxtPlugin((nuxtApp) => {
  const state = hintIssues()
    ...
    new PerformanceObserver((entryList) => {
      for (const entry of entryList.getEntries()) {
        ...
        if (performanceEntry.element.attributes?.getNamedItem('loading')?.value === 'lazy') {
          ...
          issues.get(performanceEntry.element).push({
